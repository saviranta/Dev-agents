...
PYEOF
  ...
  # ── 6b. ARCHITECT APPROVAL — prompt user to test, then create PRs ──────────
  APPROVAL="$SIGNALS/cycle.approved.json"
  if [ -f "$APPROVAL" ]; then
    APPROVAL_NOTES=$(python3 -c "import sys,json; print(json.load(open('$APPROVAL')).get('notes',''))")
    echo ""
    echo "$(ts) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "$(ts) ✓  ARCHITECT APPROVED"
    [ -n "$APPROVAL_NOTES" ] && echo "$(ts)    Notes: $APPROVAL_NOTES"
    echo "$(ts) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo "     Test the feature. Review agent-workspace/*/output/ files."
    echo "     Press Enter to create PRs, or type 'skip' to defer: "
    read -r USER_INPUT < /dev/tty

    rm -f "$APPROVAL"

    if [ "$USER_INPUT" != "skip" ]; then
      echo "$(ts) Creating PRs for completed tasks..."
      python3 << PYEOF
...
tasks = [t for t in m["tasks"] if t["status"] in ("done", "reviewed") and t.get("branch")]
created = []

for task in tasks:
    branch = task["branch"]
    first_line = (task.get("input") or "").split("\n")[0][:72]
    title = f"{task['id']}: {first_line}"
    ...
    if pr.returncode == 0:
        ...
        print(f"  ⚠  PR failed for {branch}: {pr.stderr.strip()}")

print(f"\n  {len(created)} PR(s) created.")
PYEOF
      echo ""
      echo "$(ts)    Merge when CI passes and you are satisfied."
      echo "$(ts)    Orchestrator is now idle."
    else
      echo "$(ts)    PR creation deferred. Orchestrator will re-prompt on next Architect approval."
    fi
    echo ""
  fi
